...
if __name__ == '__main__':
    ...
    st.markdown(f"<p style='text-align: center;'>{st.session_state["qi"] + 1} of {ql + 1} questions</p>", unsafe_allow_html=True)
    ...
    with col1:
        if st.button("Previous", use_container_width=True):
            ...
            if st.session_state["qi"] < 0:
                st.session_state["qi"] = ql
    with col2:
        if st.button("Next", use_container_width=True):
            st.session_state["qi"] += 1
            if st.session_state["qi"] > ql:
                st.session_state["qi"] = 0
